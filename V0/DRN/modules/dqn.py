...
class DQN(object):
	...
	def train(self, batch_states, batch_newstates, batch_rewards, batch_actions):
		# Copy the QNetwork weights to the Target QNetwork.
		if self.num_training_steps == 0:
			print("Training starts...")
			self.qnet.copy_to(self.target_qnet)
		...
		batch_newstates = np.asarray(batch_newstates)
		batch_newstates_mask = np.ones((len(batch_states)))
