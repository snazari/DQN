...
class Model(object):
    ...
    def __init__(self, config=None, db_file=os.path.join("data", "db", "reddit.sqlite"), model_file=None):
        self._num_topic = None
        self._num_onehot_tags = None
        self.model = None
        self.model_id = None
        if config:
            self.config = config
    ...
    def save_model(self, model_id, config=None):
    ...
    def compute_tag_features(self, tag_id, steps_time, to_date=None):
        ...
        for step_time in steps_time:
            ...
            # Todo: inint average_topic None
            if average_topic:
                averages_topic += average_topic.tolist()
            else:
                averages_topic += [0] * self.num_topic
        ...
        tag_features = averages_topic + counts + [diff_time]
        return tag_features
    ...
    def compute_user_features(self, user_id, steps_time, to_date=None):
        ...
        for step_time in steps_time:
            ...
            # Todo: inint average_tag None
            if average_tag:
                averages_tag += average_tag.tolist()
            else:
                averages_tag += [0] * self.num_onehot_tags
    ...
    def compute_context_features(self, steps_time, to_date=None):
        ...
        for step_time in steps_time:
            ...
            if average_user:
                ...
                averages_user += [0] * self.num_onehot_tags
        ...
        context_features = week.tolist()[0] + [time['sin'], time['cos']] + averages_topic \
                           + averages_user
    ...
    def get_features(self, user_id, tag_id, to_date=None):
        ...
        if any(features is None for features in (tag_features, user_features, user_tag_features, context_features)):
            return None
        return user_features + tag_features + user_tag_features + context_features

    def get_read_categories(self, user_id, time_state, time_next_state, all_categories):
        categories = self._dictionary.get_read_categories(user_id, time_state, time_next_state)
        read_categories = set(all_categories).intersection(set(categories))
        dict_result = {categoria: 1 for categoria in all_categories}
        dict_categories = {categoria: 0 for categoria in read_categories}
        dict_result.update(dict_categories)
        return dict_result


    def get_id_categories(self):
        tags = []
        for tag in self.config['config_onehot']['tags']:
            tag_id = self._dictionary.get_tag_id(tag)
            tags.append(tag_id)
        return tags
    ...
    def get_size_future(self):
        steps_time = self.config['config_dqnn']['steps_time']
        count_steps_time = len(steps_time)
        len_tags = self._dictionary.get_count_used_tags(self.onehot_model_id)
        len_topic = self._dictionary.get_count_topic(self.model_id)

        size_tag_features = self.size_tag_features(count_steps_time, len_tags, len_topic)
        size_user_features = self.size_user_features(count_steps_time, len_tags, len_topic)
        size_user_tag_features = self.size_user_tag_features(count_steps_time, len_tags, len_topic)
        size_context_features = self.size_context_features(count_steps_time, len_tags, len_topic)

        size_features = size_tag_features + size_user_features + size_user_tag_features + size_context_features
        return size_features

    def size_tag_features(self, count_steps_time, len_tags, len_topic):
        len_count = count_steps_time
        len_averages_topic = len_topic * count_steps_time
        # len_average_user = len_tags * count_steps_time
        len_average_user = 0
        size_tag_features = len_count + len_averages_topic + len_average_user + 1
        return size_tag_features

    def size_user_features(self, count_steps_time, len_tags, len_topic):
        averages_tag = len_tags * count_steps_time
        averages_topic = len_topic * count_steps_time
        size_user_features = averages_tag + averages_topic
        return size_user_features

    def size_user_tag_features(self, count_steps_time, len_tags, len_topic):
        size_user_tag_features = len_topic + 1
        return size_user_tag_features

    def size_context_features(self, count_steps_time,  len_tags, len_topic):
        size_averages_topic = count_steps_time * len_topic
        size_averages_user = count_steps_time * len_tags
        size_context_features = size_averages_topic + size_averages_user + 2 + 7
        return size_context_features

    @property
    def num_topic(self):
        if self._num_topic is None:
            self._num_topic = self._dictionary.get_count_topic(self.model_id)
        return self._num_topic

    @property
    def num_onehot_tags(self):
        if self._num_onehot_tags is None:
            self._num_onehot_tags = self._dictionary.get_count_used_tags(self.onehot_model_id)
        return self._num_onehot_tags
