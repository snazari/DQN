...
class Preparer:
    def __init__(self, time_intervals, **config):
        ...
        self.curr_int = 0
        self.row_counts = []
        if isinstance(config['config_dqnn']['categories'], list):
            # Set categories
            self.generate_predictable_categories(
                categories=config['config_dqnn']['categories'])
        elif isinstance(config['config_dqnn']['categories'], int):
            # Top categories
            self.generate_predictable_categories(
                top=config['config_dqnn']['categories']
            )
        elif isinstance(config['config_dqnn']['categories'], str):
            # All categories
            pass
    ...
    def generate_predictable_categories(self, top=None, custom=None, categories=None):
        """
        ...
        if custom:
            ...
            for categorie in categories:
                self.cursor.execute('''
                            SELECT tag_id FROM tags WHERE tag = '{0}' '''.format(categorie)
                                    )
                self.list_categories.append(self.cursor.fetchall()[0])
...
if __name__ == '__main__':
    ...
    flag = True
    while flag:
        data, flag = prep.get_encode_data_to_db()
        print(len(data), flag)
