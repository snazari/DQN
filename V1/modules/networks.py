...
class QNetworkDueling(QNetwork):
	...
	def __init__(self, input_size, output_size, name):
		...
		with tf.variable_scope(self.name):
			size_filter = 8
			num_filters = 32
			self.W_conv1 = self.weight_variable([size_filter, 1, num_filters]) # 32 8x8 filters over 4 channels (frames)
			...
			num_filters = 64
			size_filter = 4
			self.W_conv2 = self.weight_variable([size_filter, last_num_filters, num_filters])
			...
			num_filters = 64
			size_filter = 3
			self.W_conv3 = self.weight_variable([size_filter, last_num_filters, num_filters])
			self.B_conv3 = self.bias_variable([num_filters])
			self.stride3 = 1

	def init_fc(self, size_layer):
		# FC layer
		self.W_fc4a = self.weight_variable([size_layer, 512])  # , fanin=11*11*32)
		self.B_fc4a = self.bias_variable([512])  # , fanin=11*11*32)

		self.W_fc4b = self.weight_variable([size_layer, 512])  # , fanin=11*11*32)
	...
	def __call__(self, input_tensor):
		...
		with tf.variable_scope(self.name):
			# input_tensor is (len_state, 1)

			self.h_conv1 = tf.nn.relu(
				tf.nn.conv1d(input_tensor, self.W_conv1, stride=self.stride1,
							 padding='VALID') + self.B_conv1)

			self.h_conv2 = tf.nn.relu(
				tf.nn.conv1d(self.h_conv1, self.W_conv2, stride=self.stride2,
							 padding='VALID') + self.B_conv2)

			self.h_conv3 = tf.nn.relu(
				tf.nn.conv1d(self.h_conv2, self.W_conv3, stride=self.stride3,
							 padding='VALID') + self.B_conv3)

			size_layer = np.prod(self.h_conv3.shape[1:]).value
			self.h_conv3_flat = tf.reshape(self.h_conv3, [-1, size_layer])
			self.init_fc(size_layer)
			...
			self.h_fc6 = self.h_fc5a_value + (
					self.h_fc5b_advantage - tf.reduce_mean(self.h_fc5b_advantage, reduction_indices=[1, ],
														   keepdims=True))
